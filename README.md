...
The UI's own chrome (buttons, labels, tooltips, the settings modal, relative call times) is fully localized too, not just the Lua side. The client sends its whole locale dict — via `lib.getLocales()` — and the active `ox:locale` key to the NUI once on load; components read `ui_*`/`time_*`/`weapon_*` keys from the `Locale`/`LOCALE_KEY` stores in `ui/src/store/stores.ts`, and `Intl.DateTimeFormat` supplies the month name for relative dates instead of twelve more translated keys per language. Alert type names (`Config.Blips` codeNames) reuse the same locale key the alert message already has — `Config.AlertTypes` filter list falls back to prettifying the raw codeName only if a server adds one with no matching key. `en.json` is always the base and other languages merge on top, so a key missing from e.g. `de.json` falls back to the English string rather than the raw key name — only a key missing from `en.json` too renders as its own name. If you add a key meant for the UI, add it to all eight files and read it via those stores, the same way `ui/src/components/CallRow.svelte` and `ui/src/utils/timeAgo.ts` do. The word "Dispatch" itself is left untranslated on purpose — it is already the accepted term in this context in every language this fork ships.
