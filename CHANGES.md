...
| `ui/src/components/CallRow.svelte` | On a call row's meta line every element was `flex-shrink-0` except the street, so on a narrow panel — once the street had shrunk away — the time and the badges after it ran out of the column and painted over the unit count beside them. The street now owns the leftover space and the line clips. |
| `ui/src/components/Menu.svelte` | Moving a unit between calls arrives as two unit-count pushes, and the Active Calls board is empty for the moment between them. The panel unmounted and remounted across that gap; the list inside it updated while the panel was mid-outro, and the row that left was pinned to `position: absolute` for the animation and then never destroyed — it stayed on top of the row that replaced it for the rest of the session. The board now holds its last non-empty state briefly, so the panel survives the gap. `html/` is the rebuilt bundle. |
